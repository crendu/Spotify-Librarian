...
# -*- coding: utf-8 -*-
"""
SpotifyTools - single entry point for SpotifySortPlaylist and SpotifyLibraryAnalysis.
...
a convenience shared front door, especially useful once packaged into one .exe (see BUILD_EXE.md).

Command-line shortcuts (skip the menu):
    python SpotifyTools.py sort            same as SpotifySortPlaylist.py
    python SpotifyTools.py sort --apply    same as SpotifySortPlaylist.py --apply
    python SpotifyTools.py analysis        same as SpotifyLibraryAnalysis.py
"""
...
if __name__ == "__main__":
    ...
    if args and args[0] == "sort":
        ...
        sys.exit(f"Unknown argument(s): {' '.join(args)}\n"
                 f"Usage: python SpotifyTools.py [sort [--apply] | analysis]")
    else:
        action = show_menu()
        if action is None:
